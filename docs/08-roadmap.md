...
## M3 — 企业增强与上线（第 8-9 周）✅ 已完成（2026-08-30）

| 任务 | 产出 | 验收标准 | 状态 |
| --- | --- | --- | --- |
| TTS 语音 | 按句合成、WS 推送、前端同步播放+高亮 | 首音频帧 ≤ 1.5s；可开关 | ✅ edge-tts 服务 + socket.io 网关 + 前端播放器（首音频约 2-3.5s，edge-tts 冷连接所致，稳态可优化） |
| 审计与看板 | audit_logs、查询接口、越权告警规则 | 授权/删除/问答全量留痕 | ✅ 登录/授权/删除/问答/反馈/越权全量留痕；查询+CSV 导出+看板接口；1h 内 10 次 403 触发告警 |
| 安全加固 | Prompt 注入检测、限流、日志脱敏、LLM 网关脱敏 | 渗透测试用例通过 | ✅ 中英文注入模式拦截（400+审计）；全局 120/分+登录 10/分+问答 20/分；日志与 LLM 出站脱敏（可配置） |
| 性能压测 | 50 并发问答、入库吞吐 | 满足 01 文档 §5 指标 | ⚠️ 详见 [10-benchmark-report.md](./10-benchmark-report.md)：50 并发 100% 成功；首 Token 受远程 LLM 制约未达 2.5s |
| 备份演练 | 备份脚本 + 恢复 SOP | 完成一次全流程恢复演练 | ✅ [11-backup-restore-sop.md](./11-backup-restore-sop.md)；四组件备份+全流程恢复演练通过 |
| 上线 | 生产 compose、TLS、监控告警、运维手册 | 上线检查单全绿 | ✅ docker-compose.prod.yml + nginx TLS + healthcheck 巡检 + [12-ops-manual.md](./12-ops-manual.md) |
